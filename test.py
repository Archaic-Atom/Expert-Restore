...
from net.model import DEUIR
import open_clip

class DEUIRModel(pl.LightningModule):
    def __init__(self):
        super().__init__()
        self.net = DEUIR(dim = 32, num_blocks = [2,3,3,2], num_refinement_blocks = 2)
        # self.net = AFSUIR(dim = 48, num_blocks = [4,6,6,4], num_refinement_blocks = 4)
...
def test_Denoise(net, dataset, sigma=15):
    ...
    with torch.no_grad():
        for ([degraded_name, task_id], degrad_patch, clean_patch, DA_CLIP_degrad_patch) in tqdm(testloader):
            degrad_patch, clean_patch = degrad_patch.cuda(), clean_patch.cuda()
            DA_CLIP_degrad_patch = DA_CLIP_degrad_patch.cuda()

            image_features, degra_features = net.DA_CLIP_model.encode_image(DA_CLIP_degrad_patch, control=True)
            degra_features = degra_features.float()

            restored = net(degrad_patch, degra_features)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=int, default=5,
                        help='0 for denoise, 1 for derain, 2 for dehaze, 3 for deblur, 4 for enhance, 5 for all-in-one (three tasks), 6 for all-in-one (five tasks), 7 for raindrop',)
    
    parser.add_argument('--task_num', type=int, default=3, help='task num')
    parser.add_argument('--gopro_path', type=str, default="/data/zzd/Datasets/UIR/DEUIR/test/deblur/", help='save path of test hazy images')
    parser.add_argument('--enhance_path', type=str, default="/data/zzd/Datasets/UIR/DEUIR/test/enhance/", help='save path of test hazy images')
    parser.add_argument('--denoise_path', type=str, default="/data/zzd/Datasets/UIR/DEUIR/test/denoise/", help='save path of test noisy images')
    parser.add_argument('--derain_path', type=str, default="/data/zzd/Datasets/UIR/DEUIR/test/derain/", help='save path of test raining images') 
    parser.add_argument('--dehaze_path', type=str, default="/data/zzd/Datasets/UIR/DEUIR/test/dehaze/", help='save path of test hazy images')
    parser.add_argument('--test_path', type=str, default="/data/zzd/Datasets/NTIRE/RainDrop/valid/", help='save path of test raindrop images')

    parser.add_argument('--output_path', type=str, default="DEUIR_Visresults/", help='output save path')
    parser.add_argument('--ckpt_name', type=str, default="DEUIR_three_degradation.ckpt", help='checkpoint save path')
    ...
    net  = DEUIRModel().load_from_checkpoint(ckpt_path).cuda()
    net.eval()

    if testopt.task_num == 1:
        denoise_splits = ["bsd68/", "urban100/", "kodak24/"]
    else:
    ...
    for i in denoise_splits:
        ...
        denoise_testset = DenoiseTestDataset(testopt, net.DA_CLIP_preprocess)
    ...
    if testopt.mode == 0:
        ...
        for name in derain_splits:
            ...
            derain_set = DerainDehazeDataset(testopt,net.DA_CLIP_preprocess,addnoise=False,sigma=15)
        ...
        derain_set = DerainDehazeDataset(testopt,net.DA_CLIP_preprocess,addnoise=False,sigma=15, task='deblur')
        ...
        derain_set = DerainDehazeDataset(testopt,net.DA_CLIP_preprocess,addnoise=False,sigma=15, task='enhance')
        ...
        for name in derain_splits:
            ...
            print('Start testing {} rain streak removal...'.format(name))
            testopt.derain_path = os.path.join(derain_base_path,name)
            derain_set = DerainDehazeDataset(testopt,net.DA_CLIP_preprocess,addnoise=False,sigma=55)
        ...
        for name in deblur_splits:
            ...
            # print('Start testing {} rain streak removal...'.format(name))
            testopt.gopro_path = os.path.join(deblur_base_path,name)
            deblur_set = DerainDehazeDataset(testopt,net.DA_CLIP_preprocess,addnoise=False,sigma=55, task='deblur')
        ...
        for name in enhance_splits:
            ...
            print('Start testing LOL...')
            testopt.enhance_path = os.path.join(enhance_base_path,name)
            derain_set = DerainDehazeDataset(testopt,net.DA_CLIP_preprocess,addnoise=False,sigma=55, task='enhance')
